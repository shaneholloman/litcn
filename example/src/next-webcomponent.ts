...
export function getLiteCNMetrics() {
...
export function printLiteCNMetrics() {
   const metrics = getLiteCNMetrics();
   if (!metrics.length) {
      console.info("[litcn] no metrics recorded");
      return;
   }
   console.table(
      metrics.map((m) => ({
...
if (typeof window !== "undefined") {
   (window as any).liteCN = (window as any).liteCN || {};
   (window as any).liteCN.printMetrics = printLiteCNMetrics;
   (window as any).liteCN.enableDebug = enableLiteCNDebug;
}
...
function logDebug(factory: LogFactory) {
   ...
   console.info(`[litcn] ${message}`, ...args);
}
