# litcn Web Component Runtime Overview
...
- Debug logging can be toggled via `enableLiteCNDebug(true)` to trace what the runtime is doing.
- Parsing and interpretation timings are stored in `getLiteCNMetrics()`, so you can inspect performance after exercising the UI.
...
export function enableLiteCNDebug(value = true) {
...
function logDebug(message: string, ...args: any[]) {
   ...
   console.debug(`[litcn] ${message}`, ...args);
}
...
```ts
import { component, defaultProps, effect, enableLiteCNDebug, html, ref, signal } from "../next-webcomponent.js";

enableLiteCNDebug(true);
...
const app = html`
   <main class="min-h-screen w-full p-8 bg-background text-foreground font-sans">
      <header class="space-y-2 mb-12">
         <h1 class="text-4xl font-bold tracking-tight">litcn Web Components</h1>
